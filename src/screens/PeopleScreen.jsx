...
// people is now [{ name, venmo }]
function PersonRow({ person, index, onUpdate, onRemove, nameInputRef }) {
  const [showVenmo, setShowVenmo] = useState(!!person.venmo);
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      {/* Venmo handle row (expandable) */}
      {(showVenmo || person.venmo) && (
        <div className="flex items-center gap-2 px-4 pb-3 pt-0.5 border-t border-gray-50">
          <span className="text-gray-400 text-sm font-medium">@</span>
          <input
            type="text"
            value={person.venmo}
            onChange={e => onUpdate(index, { venmo: e.target.value.replace(/^@/, '') })}
          ...
          />
          {person.venmo && (
            <svg width="14" height="14" fill="none" stroke="#22c55e" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
export default function PeopleScreen({ people, setPeople, onNext, onBack }) {
  ...
  const addPerson = (name = '') => {
    setPeople(prev => [...prev, { name, venmo: '' }]);
    setJustAdded(true);
  };
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <div className="flex-1 px-4 py-4 space-y-4 max-w-lg mx-auto w-full pb-36">
        <p className="text-sm text-gray-500 text-center">
          Add a Venmo handle to enable payment requests later
        </p>
        ...
        <div className="space-y-2">
          {people.map((person, idx) => (
            <PersonRow
              ...
              nameInputRef={idx === people.length - 1 ? lastInputRef : null}
            />
          ))}
        </div>
        ...
        {validPeople.length === 1 && (
          <p className="text-center text-xs text-amber-600">Add at least one more person to continue</p>
        )}
      </div>
