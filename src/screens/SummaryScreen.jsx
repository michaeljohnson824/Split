...
import { generatePersonImage } from '../utils/shareImage'
...
function PersonCard({ person, colorScheme, hostVenmo, localVenmo, onVenmoChange }) {
  ...
  // Charge link: opens Venmo on the host's phone to request money FROM this person
  ...
  const venmoDeepLink = venmoHandle
    ...
    : null;
  const venmoWebLink = venmoHandle
  ...
  const handleShare = async () => {
    ...
    setIsSharing(true);
    try {
      // Generate image via Canvas API (reliable on iOS Safari)
      const dataUrl = generatePersonImage(person, colorScheme.hex, hostVenmo);

      // Convert data URL → Blob → File
      const res = await fetch(dataUrl);
      const blob = await res.blob();
      const filename = `splittab-${(person.name || 'person').toLowerCase().replace(/\s+/g, '-')}.png`;
      const file = new File([blob], filename, { type: 'image/png' });

      // Build share text — includes Venmo @ in the message box
      const shareText = hostVenmo
        ? `Here's your share of the bill: ${formatCurrency(person.total)}\nVenmo me @${hostVenmo} 🍽️`
        : `Here's your share of the bill: ${formatCurrency(person.total)} 🍽️`;

      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: `SplitTab — ${person.name}'s bill`,
          text: shareText,
        });
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            ...
            {!expanded && <span className="text-gray-400"> · tap to expand</span>}
          </p>
        </div>
      ...
      {expanded && (
        <div className="border-t border-gray-100">
          ...
          <div className="px-4 pb-4 pt-2 space-y-2">
            {/* Inline Venmo handle entry if not set */}
            {!venmoHandle && (
            ...
            <div className="flex gap-2">
              ...
              {venmoDeepLink ? (
                <a
                  href={venmoDeepLink}
                  onClick={() => {
                    setTimeout(() => { window.location.href = venmoWebLink; }, 1500);
                  }}
                ...
                <button disabled className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-gray-100 text-gray-400 rounded-xl font-semibold text-sm cursor-not-allowed">
                  <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M19.137 2.5c.607 1.012.879 2.051.879 3.365 0 4.195-3.578 9.639-6.485 13.482H7.687L5.5 3.503l5.354-.505 1.201 9.527c1.118-1.842 2.499-4.74 2.499-6.72 0-1.082-.187-1.817-.46-2.421L19.137 2.5z"/>
                  </svg>
                  Venmo
...
export default function SummaryScreen({ items, people, assignments, tax, tip, venmoUsername, onBack, onStartOver }) {
  ...
  // Allow entering / overriding Venmo handles on the summary screen itself
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <div className="flex-1 px-4 py-4 space-y-3 max-w-lg mx-auto w-full pb-36">
        ...
        <p className="text-xs text-gray-400 text-center">
          Tap a card to expand · Share sends a breakdown image with your Venmo @
        </p>
        ...
        {splits.map((person, idx) => (
          <PersonCard
            ...
            onVenmoChange={val => setLocalVenmos(prev => ({ ...prev, [idx]: val }))}
          />
        ))}
